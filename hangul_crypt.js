...
(function (root, factory) {
  ...
  var ORDERS = [0, 1, 2, 3, 4, 6, 8];
  var NM = ORDERS.length + 1;
  var MASK = (1 << 20) - 1;
  ...
  Model.prototype._setCtx = function () {
    ...
    this.h[NM - 1] = Math.imul(this.wh, GOLDEN_LO) & MASK;
  };

  Model.prototype.predict = function () {
    var c0 = this.c0;
    var w = this.w[(c0 & 255) | ((this.hist & 128) ? 256 : 0)];
  ...
  Model.prototype.update = function (bit) {
    ...
    var w = this.w[(this.c0 & 255) | ((this.hist & 128) ? 256 : 0)];
    var tgt = bit << 16;
    for (var i = 0; i < NM; i++) {
  ...
  function snapshot(m) {
    return {
      ...
      c0: m.c0, hist: m.hist, wh: m.wh, pr: m.pr, _ai: m._ai, _aw: m._aw
    };
  }

  function primedModel() {
    if (_primeCache === null) {
      var m = new Model();
      ...
      _primeCache = snapshot(m);
    }
    return snapshot(_primeCache); // 캐시 자체가 오염되지 않도록 복사본을 준다
  }
  ...
  function restore(s) {
    ...
    m.c0 = s.c0; m.hist = s.hist; m.wh = s.wh; m.pr = s.pr; m._ai = s._ai; m._aw = s._aw;
    return m;
  }

  function newPrimed() { return restore(primedModel()); }
  ...
  async function encrypt(text, password, opts) {
    ...
    var hdr = new Uint8Array([
      mid | (SEED_OPT.indexOf(seedLen) << 2) | (TAG_OPT.indexOf(tagLen) << 4) | (keyed ? 0x40 : 0)
    ]);
  ...
  return {
    ...
    _prime: function () { return newPrimed(); }
  };
});
